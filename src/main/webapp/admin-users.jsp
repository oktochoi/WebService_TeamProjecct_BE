            ...
            <div class="px-8 py-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="<%= request.getContextPath() %>/admin/logout" class="px-4 py-2 text-sm text-red-400 hover:text-red-300 transition-colors cursor-pointer">
                            <i class="ri-logout-box-line"></i> 관리자 로그아웃
                        </a>
        ...
        <div class="content-wrap">
            <div class="max-w-7xl mx-auto">
                ...
                <c:if test="${not empty param.error}">
                    <div class="mb-4 p-3 rounded-lg bg-red-900/20 border border-red-800 text-red-200">
                                        ...
                                        <c:forEach var="user" items="${users}">
                                            <tr>
                                                ...
                                                <td class="px-4 py-3 text-sm text-gray-200">
                                                    ${fn:escapeXml(user.username != null ? user.username : (user.email != null ? user.email : '-'))}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-gray-200">
                                                    ${fn:escapeXml(user.email != null ? user.email : '-')}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-gray-200">
                                                    ${fn:escapeXml(user.userRole != null ? user.userRole : '-')}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-gray-200">
                                                ...
                                                <td class="px-4 py-3 text-sm text-gray-200">
                                                    <c:choose>
                                                        ...
                                                        <c:otherwise>
                                                            <form method="post" action="<%= request.getContextPath() %>/admin/users/delete" onsubmit="return confirm('정말 삭제하시겠습니까?');" style="display:inline-block;">
                                                                <input type="hidden" name="userId" value="${user.id}" />
                                                                <button type="submit" class="px-3 py-1 rounded bg-red-600 hover:bg-red-700 text-white text-sm transition-colors cursor-pointer">
                                                                    삭제
                                                                </button>
                                                            </form>
                                                        </c:otherwise>
