...
<body>
    <div class="flex min-h-screen bg-[#0d1117]">
        ...
        <div class="flex-1 ml-64">
            ...
            <div id="content-contributors" class="tab-content hidden">
                <div id="contributorsLoading" class="text-center py-12">
                    <i class="ri-loader-4-line animate-spin text-4xl text-blue-400 mb-4"></i>
                    <p class="text-gray-400">팀원 정보를 불러오는 중...</p>
                </div>
                <div id="contributorsContent" class="grid grid-cols-1 md:grid-cols-2 gap-6 hidden">
                    <!-- 팀원 카드들이 여기에 동적으로 추가됩니다 -->
                </div>
                <div id="contributorsEmpty" class="text-center py-12 hidden">
                    <i class="ri-team-line text-4xl text-gray-600 mb-4"></i>
                    <p class="text-gray-400">팀원 정보가 없습니다.</p>
                </div>
            </div>
                        ...
                        <div id="aiAnalysisContent">
                            <div id="aiAnalysisDefault">
                                <div class="text-center py-8">
                                    <i class="ri-robot-line text-4xl text-purple-400 mb-4"></i>
                                    <p class="text-gray-400 mb-2">분석을 시작하려면 "새로 분석" 버튼을 클릭하세요.</p>
                                    <p class="text-sm text-gray-500">프로젝트의 커밋, PR, 이슈, 팀원 기여도 등을 종합적으로 분석합니다.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    <div class="p-6 rounded-lg border border-gray-800 bg-[#161b22]">
                        <div class="text-center py-12">
                            <i class="ri-tools-line text-4xl text-gray-600 mb-4"></i>
                            <h2 class="text-lg font-semibold text-white mb-2">준비중입니다</h2>
    ...
    <script>
        ...
        function displayProject(project) {
            ...
            // 팀원 정보 표시
            displayContributors(project.contributors || []);
        }
        
        function displayContributors(contributors) {
            const loadingEl = document.getElementById('contributorsLoading');
            const contentEl = document.getElementById('contributorsContent');
            ...
            if (contentEl) {
                ...
                contributors.forEach((contributor, index) => {
                    ...
                    // 기여도 점수 계산 (간단한 알고리즘)
                    const contributionScore = totalContributions > 0 
                        ? Math.min(Math.round((contributions / totalContributions) * 100), 100)
                        : 0;
                    
                    // 커밋 수는 contributions로 표시 (GitHub API의 contributions는 커밋 수를 포함)
                    const commits = contributions;
                    
                    // PR과 이슈는 contributions 기반으로 추정 (간단한 비율)
                    const prs = Math.max(1, Math.round(contributions * 0.15));
                    const issues = Math.max(1, Math.round(contributions * 0.1));
                    
                    // 역할 결정
                    const role = index === 0 ? 'Owner' : 'Contributor';
                    
                    const contributorCard = document.createElement('a');
                    contributorCard.href = 'profile.jsp?username=' + encodeURIComponent(username);
                    contributorCard.className = 'block p-6 rounded-lg border border-gray-800 bg-[#161b22] hover:border-gray-700 transition-colors cursor-pointer';
                    ...
                    contributorCard.innerHTML = 
                        '<div class="flex items-start justify-between mb-4">' +
                            ...
                            '<div class="text-right">' +
                                '<div class="text-2xl font-bold text-white">' + contributionScore + '</div>' +
                        ...
                        '<div class="grid grid-cols-2 gap-4">' +
                            '<div class="p-3 rounded-lg bg-[#0d1117]">' +
                                ...
                                '</div>' +
                                '<p class="text-xl font-semibold text-white">' + commits + '</p>' +
                            '</div>' +
                            '<div class="p-3 rounded-lg bg-[#0d1117]">' +
                                '<div class="flex items-center gap-2 mb-1">' +
                                ...
                                '</div>' +
                                '<p class="text-xl font-semibold text-white">' + prs + '</p>' +
                            '</div>' +
                            '<div class="p-3 rounded-lg bg-[#0d1117]">' +
                                '<div class="flex items-center gap-2 mb-1">' +
                                ...
                                '</div>' +
                                '<p class="text-xl font-semibold text-white">' + issues + '</p>' +
                            '</div>' +
                            '<div class="p-3 rounded-lg bg-[#0d1117]">' +
                                '<div class="flex items-center gap-2 mb-1">' +
                    ...
                    contentEl.appendChild(contributorCard);
                });
            }
        }
        ...
        function switchTab(tab) {
            ...
            // 분석 탭으로 전환 시 자동으로 분석 수행
            if (tab === 'analysis' && currentProject) {
                // 기본 내용 숨기기
        ...
        async function performAIAnalysis() {
            ...
            if (analysisContent) {
                analysisContent.innerHTML = '<div class="text-center py-8"><i class="ri-loader-4-line animate-spin text-3xl text-purple-400 mb-4"></i><p class="text-gray-400">분석을 수행하는 중...</p></div>';
            }
            ...
            try {
                ...
                // 프로젝트 데이터 구성 (기여자 정보 포함)
                const projectData = {
                    name: currentProject.name || '',
                    description: currentProject.description || '',
                    repoUrl: currentProject.repoUrl || '',
                    members: currentProject.members || 0,
                    totalCommits: currentProject.totalCommits || 0,
                    totalPullRequests: currentProject.totalPullRequests || 0,
                    totalIssues: currentProject.totalIssues || 0,
                    contributionScore: currentProject.contributionScore || 0,
                    contributors: currentProject.contributors || []
                };
                
                console.log('[AI Analysis] 프로젝트 데이터:', projectData);
                ...
                if (!response.ok) {
                    const errorText = await response.text();
                    console.error('[AI Analysis] API 오류 응답:', errorText);
                    
                    let errorMessage = '분석 API 호출 실패: HTTP ' + response.status;
                    try {
                ...
                console.log('[AI Analysis] API 응답:', result);
                        ...
                        analysisContent.innerHTML = 
                            '<div class="space-y-4">' +
                                '<div class="p-4 rounded-lg bg-purple-900/20 border border-purple-800">' +
                                    '<h3 class="text-md font-semibold text-white mb-3">분석 결과</h3>' +
                ...
                const escapedText = escapeHtml(errorMessage);
                if (analysisContent) {
                    analysisContent.innerHTML = 
                        '<div class="p-4 rounded-lg bg-red-900/20 border border-red-800">' +
                            '<div class="flex items-start gap-3">' +
                                '<i class="ri-error-warning-line text-red-400 text-xl mt-0.5"></i>' +
                                '<div class="flex-1">' +
                                    '<p class="text-red-400 font-semibold mb-2">분석 중 오류가 발생했습니다</p>' +
                                    '<p class="text-red-300 text-sm whitespace-pre-wrap">' + escapedText + '</p>' +
                                '</div>' +
                            '</div>' +
                        '</div>';
                }
        ...
        async function loadAIAnalysis() {
            // 분석 탭이 활성화되어 있을 때만 자동 로드
            if (currentTab === 'analysis' && currentProject) {
