...
<aside class="w-64 bg-[#161b22] border-r border-gray-800 fixed h-screen flex flex-col">
    ...
    <nav class="p-4 flex-1 overflow-y-auto">
        <div class="mb-6">
            ...
            <%
                // Determine if current session user is ADMIN. If admin, hide project/team/stat links.
                boolean isAdmin = false;
                if (session != null && session.getAttribute("loginUser") != null) {
                    try {
                        hello.webservice_project_be.model.User _lu = (hello.webservice_project_be.model.User) session.getAttribute("loginUser");
                        if (_lu != null && _lu.getUserRole() != null) {
                            String _r = _lu.getUserRole();
                            if ("ADMIN".equalsIgnoreCase(_r) || "ROLE_ADMIN".equalsIgnoreCase(_r)) {
                                isAdmin = true;
                            }
                        }
                    } catch (Exception _e) {
                        // ignore casting issues; default to non-admin
                    }
                }
            %>
